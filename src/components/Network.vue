...
export default {
  ...
  methods: {
    ...
    play: function(rate) {
      if (this.state != 'paused' && this.state != 'initializing') return

      if (Data.step != null) Data.step = null

      if (rate == null) rate = Data.config.simulation.step
    ...
    reset: function() {
      if (!_.isNull(Data.network)) Data.network.destroy()
      ...
      var $set = {
        node: {
          size: function(node) {
            ...
            if (
              Data.config.simulation.infectBy === 'node' ||
              Data.config.simulation.infectBy === 'both'
            )
              node.base = node.size +=
                node.rate.infect * Data.const.node.sizeFactor
          },
          borderWidth: function(node) {
            node.borderWidth = node.borderWidthSelected =
              Data.config.vis.nodes.borderWidth +
              node.rate.resist * Data.const.node.widthFactor
          },
          label: function(node) {
            if (
              Data.config.simulation.infectBy === 'node' ||
              ...
              node.label =
                _.round(node.rate.infect * 100, Data.const.node.ratePrecision) +
                '%'
          },
        },
        ...
        edge: {
          width: function(edge) {
            ...
            if (
              Data.config.simulation.infectBy === 'edge' ||
              Data.config.simulation.infectBy === 'both'
            )
          ...
          label: function(edge) {
            if (
              ...
              edge.label =
                _.round(edge.rate.infect * 100, Data.const.edge.ratePrecision) +
                '%'
          },
        },
      }
    ...
    setStats: function() {
      ...
      this.susceptible = this.filterNodesByGroup(
        Data.const.status.SUSCEPTIBLE
      ).length
      this.infected = this.filterNodesByGroup(Data.const.status.INFECTED).length
      this.recovered = this.filterNodesByGroup(
        Data.const.status.RECOVERED
      ).length
      this.vaccinated = this.filterNodesByGroup(
        Data.const.status.VACCINATED
      ).length
    ...
    allowed: function(target, action) {
      if (target.animating) return false

      if (Data.config.simulation[target.group][action]) return true

      return false
    },

    isGroup: function(node, ref) {
      if (node.group === ref) return true

      return false
    },

    mode: function(value) {
      if (Data.config.simulation.mode == value) return true
    ...
    infestation: function() {
      ...
      if ($infectBy === 'edge' || $infectBy === 'both') {
        ...
        var edges = Data.edges.get({
          filter: function(edge) {
            ...
            return (
              ($self.allowed(from, 'mayInfect') &&
                $self.allowed(to, 'mayBeInfected')) ||
              ($self.allowed(to, 'mayInfect') &&
                $self.allowed(from, 'mayBeInfected'))
            )
          },
        })
        ...
        neighbors.forEach(infected => {
          ...
          if (Math.random() > Data.config.simulation[target.group].base.resist)
            if (Math.random() > target.rate.resist)
              if (
                Math.random() <
                equation * Data.config.simulation[infected.group].base.infect
              )
        ...
        neighbors.forEach(target => {
          ...
          if (Math.random() > Data.config.simulation[target.group].base.resist)
            if (Math.random() > target.rate.resist)
              if (
                Math.random() <
                infected.rate.infect *
                  Data.config.simulation[infected.group].base.infect
              )
                infect_ani(infected, target)
        })
      }
      ...
      function infect_ani(infected, target) {
        ...
        const DOT_MOVIMENT_TIME = Number(
          Data.config.animation.infect.dotMovimentTime
        )
        const INFECT_EXPAND_TIME = Number(
          Data.config.animation.infect.expandTime
        )
        const INFECT_EXPAND_DELAY = Number(
          Data.config.animation.infect.expandDelay
        )
        const INFECT_EXPAND_SCALE = Number(
          Data.config.animation.infect.expandScale
        )
        const INFECT_RETRACT_TIME = Number(
          Data.config.animation.infect.retractTime
        )
        ...
        const REST_TIME =
          ...
          INFECT_EXPAND_DELAY +
          INFECT_RETRACT_TIME +
          Number(Data.config.animation.infect.restTime) //1.5
        ...
        TweenMax.to(target, INFECT_EXPAND_TIME * Data.config.animation.scale, {
          ...
          onComplete: function() {
            ...
            var angle = Math.atan2(
              targetPos.y - infectedPos.y,
              targetPos.x - infectedPos.x
            )
            ...
            TweenMax.to(
              target,
              INFECT_RETRACT_TIME * Data.config.animation.scale,
              {
                size: target.base,
                ease: Power3.easeOut,
                onUpdate: function() {
    ...
    recoverAttempt: function(target) {
      ...
      if ($self.allowed(target, 'mayRecover')) doRecover()

      if ($self.allowed(target, 'mayGetSusceptible')) doSusceptible()

      function doRecover() {
        if (
          Math.random() <
          target.rate.recover *
            Data.config.simulation[target.group].base.recover
        )
      ...
      function doSusceptible() {
        if (
          Math.random() <
          target.rate.susceptible *
            Data.config.simulation[target.group].base.susceptible
        )
      ...
      function recover_ani(status) {
        ...
        const RECOVER_EXPAND_TIME = Number(
          Data.config.animation.recover.expandTime
        )
        const RECOVER_EXPAND_SCALE = Number(
          Data.config.animation.recover.expandScale
        )
        const RECOVER_RETRACT_TIME = Number(
          Data.config.animation.recover.retractTime
        )
        const REST_TIME =
          RECOVER_EXPAND_TIME +
          RECOVER_RETRACT_TIME +
        ...
        TweenMax.to(target, RECOVER_EXPAND_TIME * Data.config.animation.scale, {
          ...
          onComplete: function() {
            ...
            TweenMax.to(
              target,
              RECOVER_RETRACT_TIME * Data.config.animation.scale,
              {
                size: target.base,
    ...
    killAttempt: function(target) {
      ...
      if ($self.allowed(target, 'mayDie')) kill()

      function kill() {
        if (
          Math.random() <
          target.rate.death * Data.config.simulation[target.group].base.death
        ) {
          death_ani()
        }
      }
      ...
      function death_ani() {
        ...
        const DEATH_RETRACT_SCALE = Number(
          Data.config.animation.death.retractScale
        )
        const DEATH_RETRACT_TIME = Number(
          Data.config.animation.death.retractTime
        )
        const EDGE_RETRACT_TIME = Number(
          Data.config.animation.death.edgeRetractTime
        )
        const EDGE_RETRACT_SIZE = Number(
          Data.config.animation.death.edgeRetractSize
        )
        const EDGE_RETRACT_DELAY = Number(
          Data.config.animation.death.edgeRetractDelay
        )
        const BIRTH_EXPAND_TIME = Number(
          Data.config.animation.death.birthExpandTime
        )
        const BIRTH_EXPAND_DELAY = Number(
          Data.config.animation.death.birthExpandDelay
        )
        const BIRTH_EXPAND_SCALE = Number(
          Data.config.animation.death.birthExpandScale
        )
        const BIRTH_RETRACT_TIME = Number(
          Data.config.animation.death.birthRetractTime
        )
        const REST_TIME =
          DEATH_RETRACT_TIME +
          BIRTH_EXPAND_TIME +
          BIRTH_EXPAND_DELAY +
          BIRTH_RETRACT_TIME +
          Number(Data.config.animation.death.restTime) //6.9
        ...
        TweenMax.to(target, DEATH_RETRACT_TIME * Data.config.animation.scale, {
          ...
          onComplete: function() {
            //Birth animation
            if (Data.config.simulation.d.birthWhenDie) birth_ani()
          },
        })
        ...
        function birth_ani() {
          TweenMax.to(target, BIRTH_EXPAND_TIME * Data.config.animation.scale, {
            ...
            onComplete: function() {
              ...
              TweenMax.to(
                target,
                BIRTH_RETRACT_TIME * Data.config.animation.scale,
                {
                  size: target.base,
                  ease: Elastic.easeOut.config(1, 0.3),
                  onUpdate: function() {
    ...
    inoculate: function() {
      ...
      if (Data.config.simulation.inoculation.limit.percent)
        limit = _.round(
          (Data.config.simulation.inoculation.limit.quant / 100) *
            Data.population
        )
      ...
      function neighbor() {
        ...
        if (_.isEmpty(neighbors)) return random()
      ...
      function sort(nodes) {
        return _.sortBy(nodes, [
          function(node) {
            ...
            _.forEach(node.neighbors, function() {
              quant++
            })
      ...
      function vaccinate_ani(target) {
        ...
        const VACCINATE_EXPAND_TIME = Number(
          Data.config.animation.vaccinate.expandTime
        )
        const VACCINATE_EXPAND_SCALE = Number(
          Data.config.animation.vaccinate.expandScale
        )
        const VACCINATE_RETRACT_TIME = Number(
          Data.config.animation.vaccinate.retractTime
        )
        const REST_TIME =
          VACCINATE_EXPAND_TIME +
          VACCINATE_RETRACT_TIME +
        ...
        TweenMax.to(
          target,
          VACCINATE_EXPAND_TIME * Data.config.animation.scale,
          {
            ...
            onComplete: function() {
              ...
              TweenMax.to(
                target,
                VACCINATE_RETRACT_TIME * Data.config.animation.scale,
                {
                  size: target.base,
                  ease: Power2.easeOut,
                  onUpdate: function() {
    ...
    factoryUniformFormat: function() {
      ...
      function nodesFactory() {
        ...
        _.forEach($factory.node.groups, function(group) {
          ...
          if (group.percent) quant = (group.quant / 100) * Data.nodes.length
      ...
      function edgesFactory() {
        ...
        for (let i = 0; i < row; i++)
          for (let j = 0; j < line; j++) {
            const node1 = Data.nodes.get(j * row + i)
            const node2 = Data.nodes.get((j + 1) * row + i)
    ...
    factoryFullRandom: function() {
      ...
      function groupFactory() {
        ...
        for (let i = 0; i < groups.length; i++) {
          ...
          for (let j = i + 1; j < groups.length; j++) {
            ...
            let quant = _.random(
              $factory.group.connections.min,
              $factory.group.connections.max
            )
        ...
        if (!$factory.group.startingValuesByGroup.enabled)
          ...
          var samples = _.sampleSize(
            groups,
            $factory.group.startingValuesByGroup.quant
          )
        ...
        function applyStartingValues(ids) {
          _.forEach($factory.node.groups, function(group) {
            var quant = group.quant
            if (group.percent) quant = _.round((group.quant / 100) * ids.length)
        ...
        for (var i = 0; i < quant; i++) {
          ...
          Data.nodes.add({
            ...
            rate: {
              ...
              susceptible: _.random(
                rate.susceptible.min,
                rate.susceptible.max,
                true
              ),
      ...
      function addEdge(node1, node2, limited = true) {
        ...
        Data.edges.add({
          ...
          rate: {
            infect: _.random(
              $factory.edge.rate.infect.min,
              $factory.edge.rate.infect.max,
              true
            ),
          },
        })
